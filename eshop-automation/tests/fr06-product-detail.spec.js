...
test.describe('FR-06: Product Detail View - Positive Cases', () => {
  ...
  test('TC01: Should display complete product information for valid id', async ({ page }) => {
    ...
    // Lưu ý: Các locator dưới đây (.product-image, .product-name...) là giả định. 
    // Chúng ta cần thay bằng selector chính xác của DOM thực tế sau.
  ...
  for (const quantity of validQuantities) {
    test(`TC02_03: Should update cart when adding valid quantity (${quantity})`, async ({ page }) => {
      ...
      const cartBadge = page.locator('.cart-badge'); // Placeholder selector cho số lượng giỏ hàng
      ...
      // Assertion Pattern 2: State assertion (Badge giỏ hàng cập nhật)
      // Tạm thời dùng toBeVisible(), sau này có thể check toHaveText để chính xác hơn
  ...
  test('TC10: Should show not found message for invalid product id', async ({ page }) => {
    ...
    await page.goto(`/products/${notFoundId}`);
    
    // Kỳ vọng hiển thị dòng chữ thông báo không tìm thấy
    const notFoundMessage = page.locator('text="không tìm thấy" i'); 
